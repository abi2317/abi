...
// webdriver
public static WebDriver GetBrowser(String 
type) {
 if 
(type.equalsIgnoreCase("chrome")) {
 
System.setProperty("webdriver.chrome.driver","E:\\abi\\Cucu\\drivers\\chromedriver_win32\\chromedriver.exe");
 driver = new ChromeDriver();
 
 }
 else if 
(type.equalsIgnoreCase("firefox")) {
 
System.setProperty("webdriver.gecko.driver","C:\\Users\\DELL\\eclipse-workspace\\Maven_Demo\\edge\\msedgedriver.exe");
...
(options.equalsIgnoreCase("cancel")) {
 a.dismiss();
}
else if 
(options.equalsIgnoreCase("gettext")) {
 String text = a.getText();
 System.out.println(text);
 
}
else {
 System.out.println("not handle thisalert");
}
}
...
// select
public static void Select(WebElement val, String 
options, String value) {
Select s = new Select(val);
if (options.equalsIgnoreCase("value")) {
 s.selectByValue(value);
} else if 
(options.equalsIgnoreCase("index")) {
 int parseInt = 
Integer.parseInt(value);
 s.selectByIndex(parseInt);
} else if 
(options.equalsIgnoreCase("visible")) {
 s.selectByVisibleText(value);
} else if 
(options.equalsIgnoreCase("getfirstselect")) {
 s.getFirstSelectedOption();
} else if 
(options.equalsIgnoreCase("getallselect")) {
 List<WebElement> aso = 
s.getAllSelectedOptions();
 for (int i = 0; i < aso.size(); 
i++) {
 
 System.out.println("GetAll Selected :" + aso.get(i).getText());
 }
} else if 
(options.equalsIgnoreCase("multiple")) {
 boolean multiple = s.isMultiple();
 System.out.println("Select is multiple Dropdown :" + multiple);
} else if 
(options.equalsIgnoreCase("getoptions")) {
 List<WebElement> get = 
s.getOptions();
 ListIterator<WebElement> iterator =
get.listIterator();
...
//11Deselect
public static void deSelect(WebElement val,
...
(options.equalsIgnoreCase("deselectall")) {
 s.deselectAll();
 } else {
 System.out.println("InvalidSelect Options");
 }
}
